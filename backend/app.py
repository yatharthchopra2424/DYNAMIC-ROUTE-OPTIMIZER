...
class RouteOptimizer:
    ...
    def get_routes(self, start, end):
        ...
        if "routes" not in data or data["status"] != "OK":
    ...
    def compare_routes(self, routes):
        ...
        other_routes = [
            {"route": route, "reason": f"Score: {score:.2f} (Best: {best_route[1]:.2f})"}
            for route, score in scored_routes
            if route != best_route[0]
        ]

        return {"best_route": best_route[0], "other_routes": other_routes}
    ...
    def get_route(self, start, end, vehicle_type, package_weight):
        ...
        for alt_route_data in compared_routes["other_routes"]:
            ...
            alternative_routes.append({"route": alt_route, "emissions": alt_emissions})
    ...
    def get_coordinates(self, place):
        ...
        params = {"address": place, "key": self.google_maps_api_key}
        response = requests.get(url, params=params)
        data = response.json()
        if data["results"]:
